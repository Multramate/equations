...
convertList' :: Input -> Maybe Expression
convertList' input = fmap convertList (analyse' input)
...
simplifyTests = TestList
  ...
  , convertList' "1+2" ~?= return (1 + 2)
  , convertList' "1+2+3" ~?= return (App Sum [1, 2, 3])
  , convertList' "1+2+3+4" ~?= return (App Sum [1, 2, 3, 4])
  , convertList' "1+2+3+4+5" ~?= return (App Sum [1, 2, 3, 4, 5])
  , convertList' "1+((2+(3+4))+5)" ~?= return (App Sum [1, 2, 3, 4, 5])
  , convertList' "1*2" ~?= return (1 * 2)
  , convertList' "1*2*3" ~?= return (App Prd [1, 2, 3])
  , convertList' "1*2*3*4" ~?= return (App Prd [1, 2, 3, 4])
  , convertList' "1*2*3*4*5" ~?= return (App Prd [1, 2, 3, 4, 5])
  , convertList' "1*((2*(3*4))*5)" ~?= return (App Prd [1, 2, 3, 4, 5])
  , convertList' "1+2*3" ~?= return (1 + 2 * 3)
  , convertList' "1*2+3" ~?= return (1 * 2 + 3)
  , convertList' "1+2+3*4" ~?= return (App Sum [1, 2, 3 * 4])
  , convertList' "1+2*3+4" ~?= return (App Sum [1, 4, 2 * 3])
  , convertList' "1*2+3+4" ~?= return (App Sum [3, 4, 1 * 2])
  , convertList' "1+2*3*4" ~?= return (1 + App Prd [2, 3, 4])
  , convertList' "1*2+3*4" ~?= return (1 * 2 + 3 * 4)
  , convertList' "1*2*3+4" ~?= return (App Prd [1, 2, 3] + 4)
  , convertList' "1+2+3+4*5" ~?= return (App Sum [1, 2, 3, 4 * 5])
  , convertList' "1+2+3*4+5" ~?= return (App Sum [1, 2, 5, 3 * 4])
  , convertList' "1+2*3+4+5" ~?= return (App Sum [1, 4, 5, 2 * 3])
  , convertList' "1*2+3+4+5" ~?= return (App Sum [3, 4, 5, 1 * 2])
  , convertList' "1+2+3*4*5" ~?= return (App Sum [1, 2, App Prd [3, 4, 5]])
  , convertList' "1+2*3+4*5" ~?= return (App Sum [1, 2 * 3, 4 * 5])
  , convertList' "1*2+3+4*5" ~?= return (App Sum [3, 1 * 2, 4 * 5])
  , convertList' "1+2*3*4+5" ~?= return (App Sum [1, 5, App Prd [2, 3, 4]])
  , convertList' "1*2+3*4+5" ~?= return (App Sum [5, 1 * 2, 3 * 4])
  , convertList' "1*2*3+4+5" ~?= return (App Sum [4, 5, App Prd [1, 2, 3]])
  , convertList' "1+2*3*4*5" ~?= return (1 + App Prd [2, 3, 4, 5])
  , convertList' "1*2+3*4*5" ~?= return (1 * 2 + App Prd [3, 4, 5])
  , convertList' "1*2*3+4*5" ~?= return (App Prd [1, 2, 3] + 4 * 5)
  , convertList' "1*2*3*4+5" ~?= return (App Prd [1, 2, 3, 4] + 5)
  , convertList' "1+2+3+4-5" ~?= return (Bin Sub (App Sum [1, 2, 3, 4]) 5)
  , convertList' "1+2+3-4+5" ~?= return (Bin Sub (App Sum [1, 2, 3]) 4 + 5)
  , convertList' "1+2-3+4+5" ~?= return (App Sum [4, 5, Bin Sub (1 + 2) 3])
  , convertList' "1-2+3+4+5" ~?= return (App Sum [3, 4, 5, Bin Sub 1 2])
  , convertList' "1+2+3-4-5" ~?= return (Bin Sub (Bin Sub (App Sum [1, 2, 3]) 4) 5)
  , convertList' "1+2-3+4-5" ~?= return (Bin Sub (Bin Sub (1 + 2) 3 + 4) 5)
  , convertList' "1-2+3+4-5" ~?= return (Bin Sub (App Sum [3, 4, Bin Sub 1 2]) 5)
  , convertList' "1+2-3-4+5" ~?= return (Bin Sub (Bin Sub (1 + 2) 3) 4 + 5)
  , convertList' "1-2+3-4+5" ~?= return (Bin Sub (Bin Sub 1 2 + 3) 4 + 5)
  , convertList' "1-2-3+4+5" ~?= return (App Sum [4, 5, Bin Sub (Bin Sub 1 2) 3])
  , convertList' "1+2-3-4-5" ~?= return (Bin Sub (Bin Sub (Bin Sub (1 + 2) 3) 4) 5)
  , convertList' "1-2+3-4-5" ~?= return (Bin Sub (Bin Sub (Bin Sub 1 2 + 3) 4) 5)
  , convertList' "1-2-3+4-5" ~?= return (Bin Sub (Bin Sub (Bin Sub 1 2) 3 + 4) 5)
  , convertList' "1-2-3-4+5" ~?= return (Bin Sub (Bin Sub (Bin Sub 1 2) 3) 4 + 5)
  , convertList' "1*2*3*4/5" ~?= return (Bin Div (App Prd [1, 2, 3, 4]) 5)
  , convertList' "1*2*3/4*5" ~?= return (Bin Div (App Prd [1, 2, 3]) 4 * 5)
  , convertList' "1*2/3*4*5" ~?= return (App Prd [4, 5, Bin Div (1 * 2) 3])
  , convertList' "1/2*3*4*5" ~?= return (App Prd [3, 4, 5, Bin Div 1 2])
  , convertList' "1*2*3/4/5" ~?= return (Bin Div (Bin Div (App Prd [1, 2, 3]) 4) 5)
  , convertList' "1*2/3*4/5" ~?= return (Bin Div (Bin Div (1 * 2) 3 * 4) 5)
  , convertList' "1/2*3*4/5" ~?= return (Bin Div (App Prd [3, 4, Bin Div 1 2]) 5)
  , convertList' "1*2/3/4*5" ~?= return (Bin Div (Bin Div (1 * 2) 3) 4 * 5)
  , convertList' "1/2*3/4*5" ~?= return (Bin Div (Bin Div 1 2 * 3) 4 * 5)
  , convertList' "1/2/3*4*5" ~?= return (App Prd [4, 5, Bin Div (Bin Div 1 2) 3])
  , convertList' "1*2/3/4/5" ~?= return (Bin Div (Bin Div (Bin Div (1 * 2) 3) 4) 5)
  , convertList' "1/2*3/4/5" ~?= return (Bin Div (Bin Div (Bin Div 1 2 * 3) 4) 5)
  , convertList' "1/2/3*4/5" ~?= return (Bin Div (Bin Div (Bin Div 1 2) 3 * 4) 5)
  , convertList' "1/2/3/4*5" ~?= return (Bin Div (Bin Div (Bin Div 1 2) 3) 4 * 5) ]

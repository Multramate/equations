...
reduce' :: Input -> Maybe Expression
reduce' input = fmap reduce (analyse' input)
...
simplifyTests = TestList
  ...
  , reduce' "1+x" ~?= return (App Sum [1, Con 'x'])
  , reduce' "1+x+2" ~?= return (App Sum [3, Con 'x'])
  , reduce' "1+x+2+y" ~?= return (App Sum [3, Con 'x', Con 'y'])
  , reduce' "1+x+2+y+3" ~?= return (App Sum [6, Con 'x', Con 'y'])
  , reduce' "1+((x+(2+y))+3)" ~?= return (App Sum [6, Con 'x', Con 'y'])
  , reduce' "1*x" ~?= return (App Prd [1, Con 'x'])
  , reduce' "1*x*2" ~?= return (App Prd [2, Con 'x'])
  , reduce' "1*x*2*y" ~?= return (App Prd [2, Con 'x', Con 'y'])
  , reduce' "1*x*2*y*3" ~?= return (App Prd [6, Con 'x', Con 'y'])
  , reduce' "1*((x*(2*y))*3)" ~?= return (App Prd [6, Con 'x', Con 'y'])
  , reduce' "1+x*2" ~?= return (App Sum [1, App Prd [2, Con 'x']])
  , reduce' "1*x+2" ~?= return (App Sum [2, App Prd [1, Con 'x']])
  , reduce' "1+x+2*y" ~?= return (App Sum [1, Con 'x', App Prd [2, Con 'y']])
  , reduce' "1+x*2+y" ~?= return (App Sum [1, Con 'y', App Prd [2, Con 'x']])
  , reduce' "1*x+2+y" ~?= return (App Sum [2, Con 'y', App Prd [1, Con 'x']])
  , reduce' "1+x*2*y" ~?= return (App Sum [1, App Prd [2, Con 'x', Con 'y']])
  , reduce' "1*x+2*y" ~?= return (App Sum [App Prd [1, Con 'x'], App Prd [2, Con 'y']])
  , reduce' "1*x*2+y" ~?= return (App Sum [Con 'y', App Prd [2, Con 'x']])
  , reduce' "1+x+2+y*3" ~?= return (App Sum [3, Con 'x', App Prd [3, Con 'y']])
  , reduce' "1+x+2*y+3" ~?= return (App Sum [4, Con 'x', App Prd [2, Con 'y']])
  , reduce' "1+x*2+y+3" ~?= return (App Sum [4, Con 'y', App Prd [2, Con 'x']])
  , reduce' "1*x+2+y+3" ~?= return (App Sum [5, Con 'y', App Prd [1, Con 'x']])
  , reduce' "1+x+2*y*3" ~?= return (App Sum [1, Con 'x', App Prd [6, Con 'y']])
  , reduce' "1+x*2+y*3" ~?= return (App Sum [1, App Prd [2, Con 'x'], App Prd [3, Con 'y']])
  , reduce' "1*x+2+y*3" ~?= return (App Sum [2, App Prd [1, Con 'x'], App Prd [3, Con 'y']])
  , reduce' "1+x*2*y+3" ~?= return (App Sum [4, App Prd [2, Con 'x', Con 'y']])
  , reduce' "1*x+2*y+3" ~?= return (App Sum [3, App Prd [1, Con 'x'], App Prd [2, Con 'y']])
  , reduce' "1*x*2+y+3" ~?= return (App Sum [3, Con 'y', App Prd [2, Con 'x']])
  , reduce' "1+x*2*y*3" ~?= return (App Sum [1, App Prd [6, Con 'x', Con 'y']])
  , reduce' "1*x+2*y*3" ~?= return (App Sum [App Prd [1, Con 'x'], App Prd [6, Con 'y']])
  , reduce' "1*x*2+y*3" ~?= return (App Sum [App Prd [2, Con 'x'], App Prd [3, Con 'y']])
  , reduce' "1*x*2*y+3" ~?= return (App Sum [3, App Prd [2, Con 'x', Con 'y']])
  , reduce' "1+x-2" ~?= return (Bin Sub (App Sum [1, Con 'x']) 2)
  , reduce' "1-x+2" ~?= return (App Sum [2, Bin Sub 1 (Con 'x')])
  , reduce' "1+x+2-y" ~?= return (Bin Sub (App Sum [3, Con 'x']) (Con 'y'))
  , reduce' "1+x-2+y" ~?= return (App Sum [Con 'y', Bin Sub (App Sum [1, Con 'x']) 2])
  , reduce' "1-x+2+y" ~?= return (App Sum [2, Con 'y', Bin Sub 1 (Con 'x')])
  , reduce' "1+x-2-y" ~?= return (Bin Sub (Bin Sub (App Sum [1, Con 'x']) 2) (Con 'y'))
  , reduce' "1-x+2-y" ~?= return (Bin Sub (App Sum [2, Bin Sub 1 (Con 'x')]) (Con 'y'))
  , reduce' "1-x-2+y" ~?= return (App Sum [Con 'y', Bin Sub (Bin Sub 1 (Con 'x')) 2])
  , reduce' "1+x+2+y-3" ~?= return (Bin Sub (App Sum [3, Con 'x', Con 'y']) 3)
  , reduce' "1+x+2-y+3" ~?= return (App Sum [3, Bin Sub (App Sum [3, Con 'x']) (Con 'y')])
  , reduce' "1+x-2+y+3" ~?= return (App Sum [3, Con 'y', Bin Sub (App Sum [1, Con 'x']) 2])
  , reduce' "1-x+2+y+3" ~?= return (App Sum [5, Con 'y', Bin Sub 1 (Con 'x')])
  , reduce' "1+x+2-y-3" ~?= return (Bin Sub (Bin Sub (App Sum [3, Con 'x']) (Con 'y')) 3)
  , reduce' "1+x-2+y-3" ~?= return (Bin Sub (App Sum [Con 'y', Bin Sub (App Sum [1, Con 'x']) 2]) 3)
  , reduce' "1-x+2+y-3" ~?= return (Bin Sub (App Sum [2, Con 'y', Bin Sub 1 (Con 'x')]) 3)
  , reduce' "1+x-2-y+3" ~?= return (App Sum [3, Bin Sub (Bin Sub (App Sum [1, Con 'x']) 2) (Con 'y')])
  , reduce' "1-x+2-y+3" ~?= return (App Sum [3, Bin Sub (App Sum [2, Bin Sub 1 (Con 'x')]) (Con 'y')])
  , reduce' "1-x-2+y+3" ~?= return (App Sum [3, Con 'y', Bin Sub (Bin Sub 1 (Con 'x')) 2])
  , reduce' "1+x-2-y-3" ~?= return (Bin Sub (Bin Sub (Bin Sub (App Sum [1, Con 'x']) 2) (Con 'y')) 3)
  , reduce' "1-x+2-y-3" ~?= return (Bin Sub (Bin Sub (App Sum [2, Bin Sub 1 (Con 'x')]) (Con 'y')) 3)
  , reduce' "1-x-2+y-3" ~?= return (Bin Sub (App Sum [Con 'y', Bin Sub (Bin Sub 1 (Con 'x')) 2]) 3)
  , reduce' "1-x-2-y+3" ~?= return (App Sum [3, Bin Sub (Bin Sub (Bin Sub 1 (Con 'x')) 2) (Con 'y')])
  , reduce' "1*x/2" ~?= return (Bin Div (App Prd [1, Con 'x']) 2)
  , reduce' "1/x*2" ~?= return (App Prd [2, Bin Div 1 (Con 'x')])
  , reduce' "1*x*2/y" ~?= return (Bin Div (App Prd [2, Con 'x']) (Con 'y'))
  , reduce' "1*x/2*y" ~?= return (App Prd [Con 'y', Bin Div (App Prd [1, Con 'x']) 2])
  , reduce' "1/x*2*y" ~?= return (App Prd [2, Con 'y', Bin Div 1 (Con 'x')])
  , reduce' "1*x/2/y" ~?= return (Bin Div (Bin Div (App Prd [1, Con 'x']) 2) (Con 'y'))
  , reduce' "1/x*2/y" ~?= return (Bin Div (App Prd [2, Bin Div 1 (Con 'x')]) (Con 'y'))
  , reduce' "1/x/2*y" ~?= return (App Prd [Con 'y', Bin Div (Bin Div 1 (Con 'x')) 2])
  , reduce' "1*x*2*y/3" ~?= return (Bin Div (App Prd [2, Con 'x', Con 'y']) 3)
  , reduce' "1*x*2/y*3" ~?= return (App Prd [3, Bin Div (App Prd [2, Con 'x']) (Con 'y')])
  , reduce' "1*x/2*y*3" ~?= return (App Prd [3, Con 'y', Bin Div (App Prd [1, Con 'x']) 2])
  , reduce' "1/x*2*y*3" ~?= return (App Prd [6, Con 'y', Bin Div 1 (Con 'x')])
  , reduce' "1*x*2/y/3" ~?= return (Bin Div (Bin Div (App Prd [2, Con 'x']) (Con 'y')) 3)
  , reduce' "1*x/2*y/3" ~?= return (Bin Div (App Prd [Con 'y', Bin Div (App Prd [1, Con 'x']) 2]) 3)
  , reduce' "1/x*2*y/3" ~?= return (Bin Div (App Prd [2, Con 'y', Bin Div 1 (Con 'x')]) 3)
  , reduce' "1*x/2/y*3" ~?= return (App Prd [3, Bin Div (Bin Div (App Prd [1, Con 'x']) 2) (Con 'y')])
  , reduce' "1/x*2/y*3" ~?= return (App Prd [3, Bin Div (App Prd [2, Bin Div 1 (Con 'x')]) (Con 'y')])
  , reduce' "1/x/2*y*3" ~?= return (App Prd [3, Con 'y', Bin Div (Bin Div 1 (Con 'x')) 2])
  , reduce' "1*x/2/y/3" ~?= return (Bin Div (Bin Div (Bin Div (App Prd [1, Con 'x']) 2) (Con 'y')) 3)
  , reduce' "1/x*2/y/3" ~?= return (Bin Div (Bin Div (App Prd [2, Bin Div 1 (Con 'x')]) (Con 'y')) 3)
  , reduce' "1/x/2*y/3" ~?= return (Bin Div (App Prd [Con 'y', Bin Div (Bin Div 1 (Con 'x')) 2]) 3)
  , reduce' "1/x/2/y*3" ~?= return (App Prd [3, Bin Div (Bin Div (Bin Div 1 (Con 'x')) 2) (Con 'y')]) ]
